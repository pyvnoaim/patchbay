...
- **The find bar is the second thing deliberately *not* in `OVERLAYS()`**, for the update pill's reason: `modalOpen()` would hand the window's chords to a find box and shrink a web tab away every time you looked through a shell. It lives inside `#terms`, so the pane hides it, and it takes Escape on its own input. Its chord is ⌘F on macOS and **Ctrl+Shift+F elsewhere** - plain Ctrl+F is readline's forward-char, and a session owns the keyboard.
