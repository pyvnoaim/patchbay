...
- **Releasing is a tag.** `npm run bump 0.1.0`, commit, `git tag v0.1.0`, push it: `.github/workflows/release.yml` builds macOS arm64 and x64, Windows and Linux, and puts them in a **draft** release with a `latest.json`, and a last `publish` job lets it out once all four legs succeeded. Publishing is the moment every installed copy starts offering the update, so a failed leg leaves the draft unpublished rather than offering a release one platform is missing from - a draft is not `releases/latest`, so nothing is offered until it publishes. The first job compares the tag against `tauri.conf.json` and fails in seconds rather than ten minutes into four runners.
