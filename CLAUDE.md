...
- `src-tauri/src/licence.rs` - the team licence, **gating nothing at the moment**. The shared list is free for now: `writable_list()` in `commands/mod.rs` is `list_file()` with a name, and it is the seam the wall stood in, since every writer already goes through it. The window no longer asks for a key either (the Settings ▸ Team block and `renderLicence` went with the gate); `licence.rs`, its tests and the `licence` / `save_licence` commands stay, because the argument for how it works is the part worth keeping. A licence is a signed blob beside the config (`licence`, like `web_trusted`), payload above a `--` line and a minisign signature below, and the bytes above that line are exactly what was signed - nothing may reformat them. Verification is offline on purpose: an activation ping would be the only outbound request patchbay makes. The public half is baked in; the private half is `~/.tauri/patchbay-licence.key` and is deliberately *not* the updater's, so a leaked licence key never means signed releases. `npm run licence "Acme GmbH" 2027-09-07` prints one. The check lives in public source and can be cut out - it is a compliance mechanism, not a lock, and the argument for it is that companies do not ship patched binaries.
...
npm run licence "Acme GmbH" 2027-09-07   # one signed team licence, printed
```
...
- **Only the palette ranks by recency.** Every open (`connect`, web, RDP, VNC, files) calls `used(name)`, which keeps the last forty names in `localStorage` - a habit, not part of the list, so it never touches the config. The list stays in file order: rows moving under someone reading them is a worse list, not a smarter one. `theme.js` uses `localStorage` for the same reason, and those two are the only things it holds.
