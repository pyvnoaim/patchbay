...
5. **Then release.** If `## Unreleased` in `CHANGELOG.md` has entries, `npm run bump` (next patch), commit `release x.y.z`, tag `vx.y.z`, push `main` and the tag. CI builds all four and publishes, and every installed copy offers the update on its next check. A push with nothing someone would notice releases nothing: an update that changes nothing still costs everyone a restart.
...
- **Releasing is a tag.** `npm run bump 0.1.0`, commit, `git tag v0.1.0`, push it: `.github/workflows/release.yml` builds macOS arm64 and x64, Windows and Linux, and puts them in a **draft** release with a `latest.json`, an `install` job installs the draft's own files on each OS (the `.dmg` has to pass `codesign --strict`, or a download is "damaged" with no Open Anyway - that is what `signingIdentity: "-"` is for), and a last `publish` job lets it out once every one passed and `latest.json` has a signed entry per platform. Publishing is the moment every installed copy starts offering the update, so a failed leg leaves the draft unpublished rather than offering a release one platform is missing from - a draft is not `releases/latest`, so nothing is offered until it publishes. The first job compares the tag against `tauri.conf.json` and fails in seconds rather than ten minutes into four runners.
