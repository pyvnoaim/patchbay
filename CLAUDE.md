...
- `src-tauri/src/clipboard.rs` - the CLIPRDR backend behind `rdp_session.rs`. Text, and files as CLIPRDR file lists and streams. Local to remote is lazy, as CLIPRDR is; **remote files are fetched the moment they are copied**, into a temp folder that then goes on the local clipboard, because the local clipboard can't be asked later - capped at `MAX_DOWNLOAD`. A local copy of files is walked on the session thread, capped at `MAX_ENTRIES`; the descriptors it sends and the `offered` paths behind them must stay the same length and order, because the remote asks for contents by index, which is why `describe` drops what the library would. The local side is polled (no desktop tells an unfocused process the clipboard changed), but `stamp()` asks the OS change counter first - NSPasteboard's `changeCount`, `GetClipboardSequenceNumber` - so a tick is one integer, and the text is only read once it moved. X11 has no counter, so there the text is the comparison.
- `src-tauri/src/drive.rs` - the RDPDR drive beside `clipboard.rs`: `~/Downloads/patchbay` shared as `\\tsclient\transfer`, and a drop on an RDP tab copies into it. IronRDP's native backend appends the server's path to the folder as written, so every path goes through `confine()` first; without it `\..\..\.ssh` is outside. RDPDR is only answered when RDPSND is advertised too, hence the muted one. Unix-only because that backend is.
